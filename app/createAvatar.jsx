...
function DraggableItem({ image, category, index, onDrop, isSelected, bounds }) {
  ...
  const gesture = Gesture.Pan()
    ...
    .onEnd((e) => {
      ...
      // Comprobación directa en el UI Thread usando Shared Values
      if (
        absoluteX >= bounds.x.value &&
        absoluteX <= bounds.x.value + bounds.w.value &&
        absoluteY >= bounds.y.value &&
        absoluteY <= bounds.y.value + bounds.h.value
      ) {
        runOnJS(onDrop)(category, index);
      }
  ...
  return (
    <GestureDetector gesture={gesture}>
      <Animated.View style={[styles.draggable, isSelected && styles.draggableSelected, aStyle]}>
...
export default function CreateAvatar() {
  const [selectedImages, setSelectedImages] = useState({
    color: avatarImages.color[0], cara: avatarImages.cara[0], 
    cabello: avatarImages.cabello[0], camisa: avatarImages.camisa[0], 
    short: avatarImages.short[0], pies: avatarImages.pies[0],
  });
  
  const [dropping, setDropping] = useState(false);
  
  // SHARED VALUES para las coordenadas (Evitan crasheos y son ultra rápidos)
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={{ flex: 1, backgroundColor: '#f1f5f9' }}>
        ...
        <ScrollView 
          ...
          onScrollEndDrag={onZoneLayout} // Actualiza coordenadas si el scroll movió el avatar
          onMomentumScrollEnd={onZoneLayout}
        >
            ...
            <View key={category} style={styles.categoryBlock}>
              ...
              >
                {avatarImages[category].map((image, index) => (
                  <Pressable key={index} onPress={() => setSelectedImages(p => ({...p, [category]: image}))}>
                    <DraggableItem
                      image={image}
                      category={category}
                      index={index}
                      isSelected={selectedImages[category] === image}
                      onDrop={handleDrop}
...
const styles = StyleSheet.create({
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#e2e8f0' },
  backBtn: { backgroundColor: '#e2e8f0', padding: 8, borderRadius: 8 },
  headerTitle: { fontSize: 20, fontFamily: 'PlayChickens', color: '#0c4a6e' },
  saveBtn: { backgroundColor: '#0c4a6e', padding: 10, borderRadius: 8 },
  avatarZone: { height: 230, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', borderBottomWidth: 2, borderBottomColor: '#e2e8f0' },
  avatarZoneActive: { backgroundColor: '#dbeafe', borderBottomColor: '#3b82f6' },
  avatarContainer: { width: 190, height: 190, position: 'relative' },
  avatarLayer: { width: 190, height: 190, position: 'absolute' },
  dropBadge: { position: 'absolute', top: 10, right: 10, backgroundColor: '#22c55e', width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center' },
  dropBadgeText: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  categoryBlock: { marginTop: 10, backgroundColor: '#fff', paddingTop: 10, paddingBottom: 4, borderRadius: 12, marginHorizontal: 8, marginBottom: 4, elevation: 1 },
  categoryLabel: { fontSize: 13, fontFamily: 'SlaberlinBold', color: '#64748b', paddingHorizontal: 14, marginBottom: 4 },
  draggable: { width: 86, height: 86, borderRadius: 10, overflow: 'hidden', backgroundColor: '#f8fafc', borderWidth: 2, borderColor: 'transparent' },
  draggableSelected: { borderColor: '#0369a1' },
  draggableImg: { width: 86, height: 86 },
  selectedBadge: { position: 'absolute', bottom: 4, right: 4, backgroundColor: '#0369a1', width: 16, height: 16, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  btnGuardar: { backgroundColor: '#0c4a6e', marginHorizontal: 24, paddingVertical: 16, borderRadius: 50, alignItems: 'center', marginTop: 16, marginBottom: 10 },
  btnGuardarText: { color: '#fff', fontSize: 17, fontFamily: 'SlaberlinBold' },
});